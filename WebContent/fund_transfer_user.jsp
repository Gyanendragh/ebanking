...
Connection con = null;
 try{
	...
	ResultSet rsa=statement.executeQuery("select acc_no,fname,lname from customer where username= '" + str + "'");
	if(rsa.next()){
		String name1=rsa.getString("acc_no");
		String fname=rsa.getString("fname");
		String lname=rsa.getString("lname");
		//out.print(name1);
		
	
	rsa.close();
	Statement statement1 = con.createStatement();
	ResultSet rs=statement1.executeQuery("select acc_no from customer where acc_no= '" + name1 + "'");
	...
	if(!rs1.next() ){
		...
		out.println("<script>window.location='fund_trans_user.jsp'</script>");
			...
			if(!rss.next()){
				...
				if(!rsss.next()){
				    ...
				    while( rsss1.next()){
				  	 String fname2=rsss1.getString("fname");
				   	String lname2=rsss1.getString("lname");
				    ...
				    ResultSet rss6 = statement.executeQuery("select balance from balance_info where acc_no= '" +name1+ "'");
				       ...
				       if(amount<=balance){
				    	   ...
				    	   amount-=balance;
							...
							PreparedStatement pss=con.prepareStatement("insert into balance_info(acc_no,fname,lname,balance) values('"+name2+"','"+fname2+"','"+lname2+"','"+name5+"')");
							pss.executeUpdate();
							pss.close();
							
							PreparedStatement ps=con.prepareStatement("insert into fund_transfer(acc_no_from,acc_no_to,transfer_by,remark,date_transfer,amount) values('"+name1+"','"+name2+"','"+fname+"','"+name4+"',now(),'"+name5+"')");
							ps.executeUpdate();
							ps.close();
